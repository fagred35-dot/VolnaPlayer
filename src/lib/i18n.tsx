...
const en = {
  ...
  syncSection: "Sync",
  syncScan: "Find devices",
  syncScanning: "Searching…",
  syncThisDevice: "This device",
  syncConnected: "Connected",
  syncOffline: "Offline",
  syncRename: "Rename",
  syncDisconnect: "Disconnect",
  syncBannerTitle: "Device found on network",
  syncConnect: "Connect",
  syncDismiss: "Dismiss",
  syncNone: "No devices yet",
  syncHint: "PC and phone on the same Wi-Fi",
  syncTypePc: "PC",
  syncTypePhone: "Phone",
  syncServer: "Server",
  syncServerOff: "Server not running",
  syncNoNetwork: "No Wi-Fi network",
  syncConnectByIp: "Connect by IP",
  syncIpPlaceholder: "192.168.1.5[:51789]",
  syncErrBadIp: "Invalid IP address",
  syncErrTimeout: "No answer — VPN is on, or the router blocks device-to-device traffic",
  syncErrUnreachable: "Device is unreachable — check the IP",
  syncMyIp: "My IP",
  logPanelRow: "Floating log button",
  logPanelDesc: "Floating LOG button shows the app journal — handy for diagnosing sync and wallpapers",
  logTitle: "App log",
  logCopy: "Copy",
  logClear: "Clear",
  logEmpty: "Log is empty for now",
  syncOpenPort: "Open port in firewall",
  toastPortOpened: "Port 51789 is open for sync",
  toastPortFailed: "Could not open the port — allow «Volna» through Windows Firewall",
  ...
  toastScanTruncated: "Folder is too large — only the first 5000 tracks were added",
  ...
  mcpRowTitle: "Connect MCP",
  mcpRowDesc: "Let an AI agent control the player",
  mcpRunning: "Server is running",
  mcpOff: "Server is off (VOLNA_MCP_DISABLED)",
  mcpStarting: "Server is starting…",
  mcpEndpoint: "Endpoint (Streamable HTTP)",
  mcpConfigTitle: "Config for Claude Desktop / Cursor / Cline",
  mcpCopy: "Copy",
  mcpCopied: "Copied!",
  mcpHint:
    "Paste this into the MCP config of your AI client (claude_desktop_config.json / mcp.json). The player must be running; the bridge needs Node.js. If your client supports Streamable HTTP, connect directly to the endpoint above.",
  mcpUnavailable: "MCP is available in the Windows app",
  rowStatsDesc: "Plays · time · tops",
  aboutRowTitle: "About the app",
  aboutRowDesc: "Hold the «Volna» logo — open-source projects",
  osWallpaperTitle: "Cover wallpaper",
  osWallpaperDesc: "Album art as home/lock screen wallpaper",
  osWallpaperOn: "On",
  osWallpaperTarget: "Where to apply",
  osWallpaperBlur: "Blur",
  osWallpaperOptOff: "Off",
  osWallpaperOptSystem: "Home",
  osWallpaperOptLock: "Lock",
  osWallpaperOptBoth: "Both",
  osWallpaperBlurLow: "Light",
  osWallpaperBlurMid: "Medium",
  osWallpaperBlurHigh: "Strong",
  osWallpaperNote: "Works while a track is playing; on pause the original wallpaper returns. Albums without art are skipped.",
  syncDiagBlocked: "PC found, but it doesn't answer. On the PC click «Open port in firewall»",
  syncDiagNone: "Nothing found. Check: same Wi-Fi, VPN off. On the PC click «Open port»",
  crashRowTitle: "Last crash",
  crashRowDesc: "Tap «Copy» and send it to the developer",
  crashClear: "Clear",
...
const ru: Record<TKey, string> = {
  ...
  speedTitle: "Скорость",
  shuffleShort: "Перемешать",
  repeatShort: "Повтор",
  miniPlayer: "Мини-плеер",
  syncSection: "Синхронизация",
  syncScan: "Поиск устройств",
  syncScanning: "Поиск…",
  syncThisDevice: "Это устройство",
  syncConnected: "Подключено",
  syncOffline: "Не в сети",
  syncRename: "Переименовать",
  syncDisconnect: "Отключить",
  syncBannerTitle: "Устройство в сети",
  syncConnect: "Подключить",
  syncDismiss: "Скрыть",
  syncNone: "Пока нет устройств",
  syncHint: "ПК и телефон в одной Wi-Fi сети",
  syncTypePc: "ПК",
  syncTypePhone: "Телефон",
  syncServer: "Сервер",
  syncServerOff: "Сервер не запущен",
  syncNoNetwork: "Нет Wi-Fi-сети",
  syncConnectByIp: "Подключение по IP",
  syncIpPlaceholder: "192.168.1.5[:51789]",
  syncErrBadIp: "Неверный IP-адрес",
  syncErrTimeout: "Нет ответа — включён VPN или роутер режет соединения между устройствами",
  syncErrUnreachable: "Устройство недоступно — проверьте IP",
  syncMyIp: "Мой IP",
  logPanelRow: "Плавающая кнопка журнала",
  logPanelDesc: "Кнопка LOG на экране открывает журнал приложения — удобно для диагностики синхронизации и обоев",
  logTitle: "Журнал",
  logCopy: "Копировать",
  logClear: "Очистить",
  logEmpty: "Пока пусто",
  syncOpenPort: "Открыть порт в брандмауэре",
  toastPortOpened: "Порт 51789 открыт для синхронизации",
  toastPortFailed: "Не удалось открыть порт — разрешите «Волне» доступ в брандмауэре Windows",
  ...
  toastScanTruncated: "Папка слишком большая — добавлены первые 5000 треков",
  ...
  mcpRowTitle: "Подключить MCP",
  mcpRowDesc: "Управление плеером ИИ-агентом (Claude Desktop, Cursor…)",
  mcpRunning: "Сервер работает",
  mcpOff: "Сервер выключен (VOLNA_MCP_DISABLED)",
  mcpStarting: "Сервер запускается…",
  mcpEndpoint: "Адрес (Streamable HTTP)",
  mcpConfigTitle: "Конфиг для Claude Desktop / Cursor / Cline",
  mcpCopy: "Копировать",
  mcpCopied: "Скопировано!",
  mcpHint:
    "Вставьте это в конфиг MCP вашего клиента (claude_desktop_config.json / mcp.json). Плеер должен быть запущен; для моста нужен Node.js. Если клиент умеет Streamable HTTP — подключайтесь напрямую по адресу выше.",
  mcpUnavailable: "MCP доступен в приложении для Windows",
  rowStatsDesc: "Прослушивания · время · топы",
  aboutRowTitle: "О приложении",
  aboutRowDesc: "Зажми логотип «Волна» — open-source проекты",
  osWallpaperTitle: "Обои из обложки",
  osWallpaperDesc: "Обложка трека — на рабочем столе и экране блокировки",
  osWallpaperOn: "Вкл",
  osWallpaperTarget: "Куда ставить",
  osWallpaperBlur: "Размытие",
  osWallpaperOptOff: "Выкл",
  osWallpaperOptSystem: "Стол",
  osWallpaperOptLock: "Блокировка",
  osWallpaperOptBoth: "Оба",
  osWallpaperBlurLow: "Слабое",
  osWallpaperBlurMid: "Среднее",
  osWallpaperBlurHigh: "Сильное",
  osWallpaperNote: "Работает, пока играет трек; на паузе прежние обои возвращаются. Треки без обложки пропускаются.",
  syncDiagBlocked: "ПК найден, но не отвечает. На ПК нажмите «Открыть порт в брандмауэре»",
  syncDiagNone: "Ничего не найдено. Проверьте: одна Wi-Fi сеть, VPN выключен. На ПК нажмите «Открыть порт»",
  crashRowTitle: "Последний сбой",
  crashRowDesc: "Нажмите «Копировать» и пришлите разработчику",
  crashClear: "Очистить",
